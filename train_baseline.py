...
def get_model(n_feat, n_cls):
    ...
    if args.model == 'BaselineCNN':
        ...
        model = Baseline_CNN(n_feat, n_cls, 128).to(device)
    elif args.model == 'DeepConvLSTM':
        from models.DeepConvLSTM import DeepConvLSTM
        model = DeepConvLSTM(n_feat, n_cls).to(device)
    elif args.model == 'LSTMConvNet':
        from models.LSTM_CNN import LSTMConvNet
        model = LSTMConvNet(n_feat, n_cls).to(device)
    elif args.model == 'EarlyFusion':
        from models.CAGE import CAGE_EarlyFusion
        model = CAGE_EarlyFusion(n_feat, n_cls).to(device)
    else:
...
def evaluate(model, eval_loader, epoch, is_test=True, mode='best'):
    ...
    with torch.no_grad():
        for idx, (data, label) in enumerate(eval_loader):
            ...
            output = model(data)
            total_loss = total_loss + criterion_cls(output, label) * len(label)
            _, predicted = torch.max(output, 1)
            label_list.append(label)
            predicted_list.append(predicted)
            total_num = total_num + len(label)
